...
class Searchingfilter:ObservableObject{
    var popular:PopulateListOfCombine
    
    init(popular: PopulateListOfCombine) {
        self.popular = popular
    }
    func searching(searchText:String){
        if searchText.isEmpty{
            popular.filiteredplanetsList = popular.planetsList.sorted(by: {$0.name < $1.name})
        }else{
            let newlist = popular.planetsList.filter({planet in
                      planet.name.localizedCaseInsensitiveContains(searchText)
                  })
            popular.filiteredplanetsList = newlist.sorted(by: {$0.name < $1.name})
              }
        }
